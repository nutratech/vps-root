...
URL="https://github.com/$REPO/releases/download/$VERSION/cinny-$VERSION.tar.gz"
DEST="/var/www/cinny"
TMP_DIR=$(mktemp -d)
BACKUP_DIR="/var/www/cinny_backups/$(date +%s)"

echo "Updating Cinny to $VERSION..."
echo "Download URL: $URL"

# Download
echo "Downloading..."
curl -L -o "$TMP_DIR/cinny.tar.gz" "$URL"

# Verify download
if [ ! -s "$TMP_DIR/cinny.tar.gz" ]; then
...
sudo tar -xzf "$TMP_DIR/cinny.tar.gz" --strip-components=1 -C "$STAGING_DIR"
...
echo "Update complete! Cinny is now at $VERSION."
